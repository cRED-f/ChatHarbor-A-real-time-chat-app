import Link from "next/link";
import Image from "next/image";
import home from "../assets/images/homepage/home.png";
import homeDark from "../assets/images/homepage/homeDark.png";
import { MoveRight } from "lucide-react";

export default async function Home() {
  return (
    <main>
      <div className="relative isolate  dark:bg-gray-900 ">
        {/* gloom effect */}
        <div
          className="absolute  inset-x-0 top-28 -z-10 transform-gpu overflow-hidden blur-3xl"
          aria-hidden="true"
        >
          <div
            className="relative left-[calc(50%-11rem)] aspect-[1155/678] 
            w-[36.125rem] -translate-x-1/2 rotate-30 bg-gradient-to-tr
             from-[#594b92] to-[#11dffa] opacity-30 sm:left-[calc(50%-30rem)] 
             sm:w-[72.1875rem]"
            style={{
              clipPath:
                "polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 32.5% 60.28%, 62.48% 52.48%, 68.18% 47.5%, 58.3% 45.2%, 34.5% 80.7%, 2% 72.5%, 27.5% 76.7%, 0.1% 64.98%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)",
            }}
          />
        </div>
        <div className="py-12 sm:py-20 lg:pb-40">
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="mx-auto max-w-2xl text-center">
              <h1 className="text-4xl font-bold tracking-tight sm:text-6xl">
                Chat with your friends, Anywhere!
              </h1>
              <p className="mt-6 text-lg leading-8 text-gray-600 dark:text-gray-300">
                You can speak your native language, it does not matter. <br />
                <span className="text-indigo-600 dark:text-indigo-500">
                  {" "}
                  Let AI handle Everything for you.
                </span>
              </p>
              <div className="mt-10 flex items-center justify-center gap-x-6">
                <Link
                  href="/chat"
                  className="rounded-md animate-bounce  bg-gradient-to-br from-[#2537c0]
                    to-[#2195bb] px-3.5 py-2.5 text-sm font-semibold text-white
                    dark:text-white shadow-sm hover:shadow-md hover:from-[#2537c0] hover:to-[#a62bfd]"
                >
                  Get started
                </Link>
                <Link href="/price">
                  <h1 className="dark:hover:text-indigo-600 hover:text-indigo-600  flex gap-1 text-sm  font-semibold leading-6 text-gray-900 dark:text-gray-300">
                    {" "}
                    View Pricing <MoveRight width={15} />
                  </h1>
                </Link>
              </div>
            </div>
            <div className="mt-16 sm:mt-24">
              <div className="-m-2 rounded-xl shadow-inner shadow-[#CCEBF5] block dark:hidden bg-gray-900/5 p-2 ring-1 ring-inset ring-gray-900/10 lg:-m-4 lg:rounded-2xl lg:p-4">
                <Image
                  src={home}
                  alt="App screenshot"
                  width={1243}
                  height={1442}
                  className="rounded-md shadow-2xl ring-1 ring-gray-900/10"
                />
              </div>
              <div className="-m-2 rounded-xl shadow-inner shadow-[#CCEBF5] hidden dark:block bg-gray-900/5 p-2 ring-1 ring-inset ring-gray-900/10 lg:-m-4 lg:rounded-2xl lg:p-4">
                <Image
                  src={homeDark}
                  alt="App screenshot"
                  width={1243}
                  height={1442}
                  className="rounded-md shadow-2xl ring-1 ring-gray-900/10"
                />
              </div>
            </div>{" "}
            {/* gloom effect */}
            <div
              className="absolute inset-x-0 top-[calc(100%-13rem)] -2-10 transform-gpu overflow-hidden blur-3xl sm:top-[calc(100%-30rem)]"
              aria-hidden="true"
            >
              {" "}
              <div
                className="relative left-[calc(50%+3rem)] aspect-[1155/678] w-[36.125rem]
             -translate-x-1/2 bg-gradient-to-tr from-[#594b92]
              to-[#11dffa] opacity-30 sm:left-[calc(50%+36rem)]
               sm:w-[72.1875rem]"
                style={{
                  clipPath:
                    "polygon(74.1% 44.18%, 100% 61.68%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 28, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)",
                }}
              ></div>
            </div>
          </div>{" "}
        </div>
      </div>
    </main>
  );
}
